...
| --- | --- | --- | --- |
| **Stack** | The single `angee.yaml` + generated runtime files at `ANGEE_ROOT`. | `stackStatus`, `stackLogs`. | `stackPrepare` (generated runtime configuration and environment), `stackInit`, `stackUpdate`, `stackBuild`, `stackUp`, `stackDev`, `stackDown`, `stackDestroy`. |
...
| **Job** | One-shot commands in the startup dependency graph, with daemon-owned receipts for explicit reruns. | `jobs`, `jobRunOperation`, `latestJobRun`, `jobRunPreview`. | `jobRun(name, inputs, chainedRestart)`. A chained run starts dependents only after the root succeeds, waits at dependency joins, and reports failed or blocked descendants individually. |
...
| **Secrets** | Values referenced as `${secret:name}` in the manifest. | `secrets` (declared list, metadata only), `secret(name)` (metadata), `secretValue(name)` (privileged value read), `stackPrepare` returns the resolved env-var names. | `secretSet(name, value)` / `secretDelete(name)` against the configured backend (env-file or OpenBao). Set and delete accept any valid name (declared or not); list only shows declared. Mutating ops audit-log to stderr. |
