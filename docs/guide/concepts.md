...
| **Job** | Explicitly invoked one-shot commands and their daemon-owned run receipts. | `jobs`, `jobRunOperation`, `latestJobRun`, `jobRunPreview`. | `jobRun(name, inputs, chainedRestart)`. A chained run starts dependents only after the root succeeds, waits at dependency joins, and reports failed or blocked descendants individually. |
